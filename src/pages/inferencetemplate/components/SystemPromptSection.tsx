...
export function SystemPromptSection({
  ...
  const handleUpdatePrompt = (id: string, updates: Partial<SystemPrompt>) => {
    onUpdate(prompts.map((prompt) => (prompt.id === id ? { ...prompt, ...updates } : prompt)));
  };
        ...
        <div className="space-y-1">
          {prompts.map((prompt) => (
            <SystemPromptItem
              key={prompt.id}
              prompt={prompt}
              onUpdate={(updates) => handleUpdatePrompt(prompt.id, updates)}
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="center" className="w-56">
                {availableTypes.map((type) => (
                  <DropdownMenuItem key={type} onClick={() => handleAddPrompt(type)}>
                    {type
                      .replace(/([A-Z])/g, " $1")
                      .toLowerCase()
                      .replace(/^./, (str) => str.toUpperCase())}
