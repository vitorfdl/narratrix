...
import { LuEye, LuEyeOff, LuGlobe } from "react-icons/lu";
              ...
              {field.field_type === "boolean" ? (
                ...
                <div className="relative">
                  ...
                  <Button type="button" variant="ghost" size="icon" className="absolute right-0 top-0 h-full" onClick={() => toggleSecretVisibility(field.key)}>
                    {showSecrets[field.key] ? <LuEye className="h-4 w-4" /> : <LuEyeOff className="h-4 w-4" />}
                  </Button>
                </div>
              ) : field.field_type === "url" ? (
                <div className="relative">
                  <Input className="pl-8" placeholder="https://example.com" defaultValue={field.default as string} {...formField} />
                  <LuGlobe className="absolute left-2 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground pointer-events-none" />
                </div>
              ) : (
