...
export function CharacterSidebar({
  ...
  const uniqueTags = Array.from(new Set(characters.flatMap((char) => char.tags ?? []).filter((tag): tag is string => Boolean(tag)))).sort((a, b) => a.localeCompare(b));
          ...
          <div className="rounded-xl border border-border/60 bg-background/60 p-3 shadow-sm">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
              <div className="rounded-lg bg-muted/40 px-2 py-1.5">
                ...
                <div className="text-sm font-semibold">{totalCharacters}</div>
              </div>
              <div className="rounded-lg bg-muted/40 px-2 py-1.5">
                <div className="text-muted-foreground">Shown</div>
                <div className="text-sm font-semibold">{filteredCharactersCount}</div>
              </div>
            </div>
          </div>
