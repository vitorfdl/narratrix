...
import { CharacterSheetTab } from "./sheet/CharacterSheetTab";
import { useChatSheetValues } from "./sheet/useChatSheetValues";
...
interface CharacterFormProps {
  ...
  title?: string;
  // When true (opened from a chat), sheet values are saved to the chat only and
  // the Character Sheet tab is shown first. Template edits still apply globally.
  chatSheetScope?: boolean;
}
...
export const CharacterForm = forwardRef<CharacterFormRef, CharacterFormProps>(
  ({ onSuccess, initialData, mode = "create", setIsEditing, open = false, onOpenChange, title, chatSheetScope = false }, ref) => {
    ...
    const [activeTab, setActiveTab] = useState(chatSheetScope ? "sheet" : "info");
    ...
    const [sheetTemplateId, setSheetTemplateId] = useState<string | null>(initialData?.custom?.sheet_template_id || null);
    const { overrides: chatSheetOverrides, saveOverrides: saveChatSheetValues, isChatScoped } = useChatSheetValues(initialData?.id, chatSheetScope && isEditMode);
    const [sheetValues, setSheetValues] = useState<SheetValues>(() => ({ ...(initialData?.custom?.sheet_values ?? {}), ...(chatSheetScope ? (chatSheetOverrides ?? {}) : {}) }));
    ...
    React.useEffect(() => {
      ...
      setSheetTemplateId(initialData?.custom?.sheet_template_id || null);
      setSheetValues({ ...(initialData?.custom?.sheet_values ?? {}), ...(chatSheetScope ? (chatSheetOverrides ?? {}) : {}) });
    }, [initialData?.id]);
    ...
    useEffect(() => {
      ...
      const isSheetTemplateChanged = (initialData?.custom?.sheet_template_id ?? null) !== sheetTemplateId && sheetTemplateId !== null;

      if (isNameChanged || isPersonalityChanged || isSystemPromptChanged || isTagsChanged || isAvatarChanged || isLorebookChanged || isSheetTemplateChanged) {
        setIsEditing(true);
      }
    }, [version, name, personality, systemPrompt, author, tags, avatarImage, selectedLorebookId, sheetTemplateId, initialData, setIsEditing]);
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
      ...
      try {
        ...
        const custom: Record<string, unknown> = {
          ...
          // Chat-scoped edits keep the character's own defaults untouched
          sheet_values: isChatScoped ? (initialData?.custom?.sheet_values ?? {}) : sheetValues,
        };
        ...
        if (isEditMode && initialData) {
          ...
          if (isChatScoped) {
            await saveChatSheetValues(sheetValues);
          }
        } else {
      ...
      <div className="space-y-4">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid grid-cols-3 w-full mt-4">
            {chatSheetScope && <TabsTrigger value="sheet">Character Sheet</TabsTrigger>}
          ...
          <TabsContent value="sheet" className="pt-4">
            <CharacterSheetTab
              ...
              onTemplateChange={setSheetTemplateId}
              values={sheetValues}
              onValuesChange={setSheetValues}
              characterName={name}
              scopeHint={isChatScoped ? "Values you fill here are saved to this chat only and won't change the character's defaults." : undefined}
            />
          </TabsContent>
