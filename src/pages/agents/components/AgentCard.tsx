...
import { Bot, Code2, Edit, GitBranch, Heart, HeartOff, Network, Trash2, Zap } from "lucide-react";
...
export function AgentCard({ agent, onEdit, onDelete, onToggleFavorite }: AgentCardProps) {
  ...
  // Count nodes by type
  const nodeTypes = agent.nodes.reduce((acc, node) => {
  ...
  return (
    <Card className="group relative overflow-hidden flex flex-col h-full bg-gradient-to-br from-background to-accent/10 hover:shadow-lg transition-all duration-300 border-2 hover:border-primary/50">
      <CardHeader className="relative pb-3">
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2">
            {/* Run trigger badge */}
            <Badge 
              variant={runTrigger === "every_message" ? "default" : "secondary"} 
      ...
      <CardContent className="flex-1 space-y-3">
        ...
        {tags.length > 0 && (
          <div className="flex flex-wrap gap-1">
            ...
            {tags.length > 4 && (
              <span className="text-xs text-muted-foreground font-semibold px-1">+{tags.length - 4}</span>
            )}
          </div>
        )}
      </CardContent>
