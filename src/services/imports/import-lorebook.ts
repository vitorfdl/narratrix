...
export async function importLorebook(validatedData: z.infer<typeof internalLorebookImportSchema>): Promise<Lorebook> {
  // RAG fields and stored vectors are profile-bound: a foreign embedding_model_id would resolve to another profile's encrypted API key,
  // and vectors generated by it live in a different embedding space. Strip both so the importing profile re-enables RAG manually.
