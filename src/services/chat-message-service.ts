...
export async function getChatMessagesByChatId(chatId: string): Promise<ChatMessage[]> {
  const validChatId = uuidUtils.uuid().parse(chatId);
  return listChatMessages({ chat_id: validChatId });
}

// Get the latest position for a chat to add a new message
export async function getNextMessagePosition(chatId: string): Promise<number> {
  const validChatId = uuidUtils.uuid().parse(chatId);

  const result = await selectDBQuery<{ max_position: number | null }[]>(
    "SELECT MAX(position) as max_position FROM chat_messages WHERE chat_id = $1",
    [validChatId],
  );
