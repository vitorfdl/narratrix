...
// Paths owned by the existing character macros ({{character.name}},
// {{character.personality}}, {{character.memory}} and the {{user.*}}
// equivalents); never treated as sheet references even if a sheet field
// shares the key.
const RESERVED_PATHS = new Set(["name", "personality", "memory"]);
...
// first closing braces. "char" and "character" are interchangeable; the
// reserved paths above are left for the legacy macro pass, which runs after
// this one.
...
/**
 * Replaces {{char.SECTION}}, {{char.SECTION.FIELD}} and {{char.sheet}} — also
 * spelled {{character.*}} — (or the {{user.*}} equivalents) with the
 * referenced sheet content rendered as markdown. Unresolvable references (no sheet configured, unknown
