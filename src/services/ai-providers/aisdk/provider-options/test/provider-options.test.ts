...
describe("getAWSBedrockProviderOptions", () => {
  it("uses adaptive with mapped effort for Opus 4.7", () => {
    const opts = getAWSBedrockProviderOptions({ reasoning_temperature: 2 }, OPUS_4_7);
    expect(opts.reasoningConfig).toEqual({ type: "adaptive", maxReasoningEffort: "medium" });
  });
  ...
  it("defaults adaptive effort to high when only budget is set", () => {
    ...
    expect(opts.reasoningConfig).toEqual({ type: "adaptive", maxReasoningEffort: "high" });
  });
...
describe("getAnthropicProviderOptions", () => {
  it("uses adaptive thinking with sibling effort for Opus 4.7", () => {
