...
function parseLorebookJsonFields(lorebook: any): Lorebook {
  return {
    ...
    rag_enabled: parseBoolean(lorebook.rag_enabled),
    embedding_model_id: lorebook.embedding_model_id ?? null,
    similarity_threshold: lorebook.similarity_threshold ?? 0.7,
  };
}
...
export async function createLorebook(lorebookData: CreateLorebookParams): Promise<Lorebook> {
  ...
  await executeDBQuery(
    `INSERT INTO lorebooks (
      ...
      group_keys, rag_enabled, embedding_model_id, similarity_threshold,
    ...
    [
      ...
      groupKeysStr,
      validatedLorebook.rag_enabled,
      validatedLorebook.embedding_model_id,
      validatedLorebook.similarity_threshold,
      extraStr,
      now,
      now,
    ],
...
export async function updateLorebook(id: string, updateData: UpdateLorebookParams): Promise<Lorebook | null> {
  ...
  // Clear stored vectors when the embedding model changes so the lorebook
  // doesn't retain vectors generated by a different (possibly different-dimension) model.
  const newEmbeddingModelId = updateData.embedding_model_id;
  if (newEmbeddingModelId !== undefined && newEmbeddingModelId !== currentLorebook.embedding_model_id && currentLorebook.embedding_model_id) {
...
export async function clearLorebookVectors(lorebookId: string): Promise<void> {
  const validId = uuidUtils.uuid().parse(lorebookId);
  await executeDBQuery("UPDATE lorebook_entries SET vector_content = NULL WHERE lorebook_id = $1", [validId]);
}
